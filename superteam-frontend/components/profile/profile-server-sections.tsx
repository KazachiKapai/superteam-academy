...
async function resolveIdentity(
  ...
  if (user) return getProfileIdentity(user);
  if (wallet) return getProfileIdentityByWallet(wallet);
  throw new Error("Either user or wallet must be provided");
}
...
export async function ProfileHeaderSection({
  ...
  const identity = await resolveIdentity(user, wallet);
...
export async function ActivitySection({ wallet }: { wallet: string }) {
...
export async function CompletedCoursesSection({ wallet }: { wallet: string }) {
...
export async function SkillsSection({ wallet }: { wallet: string }) {
  const snapshots = await getProfileCourses(wallet);
  const t = await getTranslations("profile");
...
export async function VisibilitySection({
  isOwnProfile = true,
}: {
  isOwnProfile?: boolean;
}) {
  ...
  return (
    <Card>
      ...
      <CardContent className="flex items-center justify-between">
        ...
        <VisibilityToggle isOwnProfile={isOwnProfile} />
      </CardContent>
    </Card>
  );
}
